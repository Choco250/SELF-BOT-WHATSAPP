...
const vcard = 'BEGIN:VCARD\n' 
            + 'VERSION:3.0\n' 
            + 'FN:Choco Ganz' //ganti nama lu! 
            + 'ORG: Owner Choco Bot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=6283894905341:+62 838-9490-5341\n'  //ganti nomor lu! 
            + 'END:VCARD'
prefix = '#'
blocked = []   
limitawal = 10000
memberlimit = 1
cr = '*CHOCO BOT WHATSAPP*'

/******** OWNER NUMBER**********/
const ownerNumber = ["6283894905341@s.whatsapp.net"]   //ganti nomor lu! 
const pacarNumber = ["6281229331235@s.whatsapp.net"]  //ganti nomor lu!  
/************************************/
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("JAGA UCAPAN DONG!! 😠")
                        ...
                        .then(() => {
                            client.sendMessage(from, `*「 ANTI BADWORD 」*\nChoco Bot akan kick kamu karena berkata kasar!`, text ,{quoted: mek})
		...
		setTimeout( () => {
			...
			switch(command) {
				case 'nobadword':  // Fix Bug By XYTODBot & Yogi*
				...
				case 'addbadword':  // Fix Bug By XYTODBot & Yogi*
                    ...
                    break
                case 'delbadword':  // Fix Bug By XYTODBot & Yogi*
                    if (!isOwner) return reply(ind.ownerb())
                    if (!isGroupAdmins) return reply(ind.admin())
                    if (args.length < 1) return reply( `Kirim perintah ${prefix}addbadword [kata kasar]. contoh ${prefix}addbadword bego`)
		...
		case 'cuttly':
				...
				case 'pornhub':
					...
					var porn = gh.split("/")[0];
					var hub = gh.split("/")[1];
					if (args.length < 1) return reply(`ã€Œâ—ã€Contoh : ${prefix}pornhub Yogi/Hub`)
					reply(ind.wait())
...
//dappa gay
	...
	break
		...
		case 'ply':   
				...
				if (anu.error) return reply(anu.error)
				infomp3 = `*Lagu Ditemukan!!!*\nJudul : ${anu.result.title}\nSource : ${anu.result.source}\nUkuran : ${anu.result.size}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM YA BOS*`
				...
				break
				case 'play':
					if (args.length == 0) return reply(`Usage: ${prefix + command} query\nExample: ${prefix + command} cidro`)
                    query = body.slice(6)
					reply('Wait proses dumlu...')
					anu = await fetchJson(`https://fzn-gas.herokuapp.com/api/ytplay2?judul=${query}`, {method: 'get'})
					txt = `「 Play Music 」\n\n• Judul : ${anu.title}\n• Durasi : ${anu.duration}\n• Size : ${anu.filesize}\n\n*Wait Audio sedang dikirim*`
					thumb = await getBuffer(anu.image)
					client.sendMessage(from, thumb, image, {caption: txt, quoted: mek})
					musiknye = await getBuffer(anu.result)
					client.sendMessage(from, musiknye, audio, {mimetype: 'audio/mp4', quoted: mek})
					break
		/*case 'play2':   
				// Fix Case By Yogi/Hans⛔
                 if (!isRegistered) return reply( ind.noregis())
		...
		case 'grouplist':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group XYTOD-BOT :\n\n\`\`\``
					no = 0
		...
		case 'menu':
            //Gak Usah Di Hapus TQTO Gblk Dimana Harga Diri Lu Gw Capek2 Ngefix malah di hapus 
                 if (!isRegistered) return reply('*DAFTAR DULU BOS*\n\nKETIK #DAFTAR (namalu)|18\n\nJIKA SUDAH TERDAFTAR KETIK #menu')
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					const reqXp  = 5000 * (Math.pow(2, getLevelingLevel(sender)) - 1)
					const uangku = checkATMuser(sender)
                    wew = fs.readFileSync(`./src/logo.jpg`)
                    yogi = `◪ INFO BOT DEVICE
...
case 'bneon':
		...
		case 'lpr':
					...
					reply('Masalah telah di laporkan ke owner XYTOD-BOT, Laporan palsu atau main² tidak akan ditanggapi.')
					break
		case 'request':
					...
					client.sendMessage('6283152753417@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('6283152753417@s.whatsapp.net', options, text, {quoted: mek})
					reply('REQUEST ANDA TELAH SAMPAI KE OWNER XYTOD-BOT, Requests palsu atau main² tidak akan ditanggapi.')
					break
		case 'blocklist': 
		...
		case 'animehug':
				...
				if (args.length < 1) return reply(ind.wrongf())
                    ...
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/photooxy1/${command}?apikey=RamlanID&text=${txt}`)
                    client.sendMessage(from, buffer, image)
                    break
                case 'wetglass':
                case 'multicolor3d':
				...
				if (args.length < 1) return reply(ind.wrongf())
                    if (args.length == 0) return reply('Teksnya mana um')
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/ephoto1/${command}?apikey=RamlanID&text=${txt}`)
                    client.sendMessage(from, buffer, image)
                    break
		case 'pokemon':
					// Fix Case By Yogi/Hans⛔
		...
		case 'hargapremium':
					...
					break
		case 'buypremium':
					client.sendMessage(from, 'Ingin membeli premium?\nHarap hubungi kami : wa.me/6283152753417',MessageType.text, { quoted: mek} )
					break
		...
		case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST XYTOD-BOT 」*\n\n${body.slice(4)}`)
						}
						reply('*Suksess broadcast* ')
					}
					break
        ...
        if (budy.includes(`payment`)) {

                  reply(`*VIA PEMBAYARAN*\n\nGOPAY : 081229331235\nPULSA : 083894905341\n\n*JIKA SUDAH TF JAN LUPA KIRIM BUKTI*`)
       ...
       if (budy.includes(`Save gw`)) {

                  reply(`Yoi, save back Choco Bot`)

                  }



       if (budy.includes(`Bang minta case`)) {

                  reply(`2021 masih ngemis case? hdehh....`)
        ...
        if (budy.includes(`cho`)) {

                  reply(`Ya?\nUntuk Menggunakan Bot Ketik #menu/help`)

                  }

       if (budy.includes(`CHOCO BOT`)) {
       ...
       if (budy.includes(`@+12513876184`)) {

                  reply(`Iyaa Apa Ada Tag Choco Bot?\nKalo mau main sama bot ketik ${prefix}menu ya ka`)

                  }
                 
       if (budy.includes(`@6283894905341`)) {

                  reply(`Napa tag owner gw? mau gw peluk?`)
       ...
       if (budy.includes(`menu`)) {
                  ...
                  reply(`KETIK ${prefix}menu UNTUK MAIN AMA BOT`)

                  }

       if (budy.includes(`Choco`)) {

                  reply(`Jangan Panggil Choco Kak, Dia Lagi Sibuk 🗣`)
		...
		if (budy.includes(`cekprefix`)) {
                  ...
                  reply(`*CHOCO BOT MENGGUNAKAN PREFIX [ ${prefix} ]*`)
       ...
       if (budy.includes(`Nice`)) {
		...
		if (budy.includes(`Makasih`)) {
			...
			console.log('[ ERORR ]%s', color(e, 'red'))
		}
	})
